...
def check_duplicate(csv_filename, new_data):
    with open(csv_filename, 'r', newline='') as file:
        ...
        if len(existing_data) > 1:
            new_data_list = [
                new_data['Transaction Type'],  # Make sure to include the transaction type
                new_data['Date'],
                new_data['Time'],
...
# Example usage
#folder_path = '/Users/ethansmith/Documents/GitHub/investments/screenshots/BUY'
csv_filename = '/Users/ethansmith/Documents/GitHub/investments/investment_data.csv'
#process_folder(folder_path, csv_filename)

def sort_csv_(csv_filename):
    with open(csv_filename, 'r', newline='') as file:
        reader = csv.DictReader(file)
        rows = list(reader)
    ...
    # Convert date and time to datetime objects and sort
    sorted_data = sorted(
        rows,
        key=lambda row: (
            datetime.strptime(row['Date'], date_format) if row['Date'] != '00-00-0000' else datetime.max,
            datetime.strptime(row['Time'], time_format) if row['Time'] != '00:00' else datetime.max
        ),
        reverse=True  # Set reverse to True for newest to oldest
    )
